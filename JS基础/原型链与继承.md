...
* 每一个函数都有一个prototype属性，protopype属性是一个对象，它只有一个内置的属性constructor，指向这个函数本身。
* 每一个对象都有一个proto属性，它执行创建这个对象的构造函数的protopype
* 当我们在对象上查找一个属性找不到时，就会去它的proto所指向的对象去寻找，直到找到，或者null。
